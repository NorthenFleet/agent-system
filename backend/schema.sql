-- ============================================================================
-- Task-Plan 数据库 Schema 变更
-- 
-- 描述：任务 (Task) 与开发计划 (Development Plan) 的从属关系数据存储
-- 设计者：李奥纳多 (🐢 架构师)
-- 创建日期：2026-04-16
-- 版本：v1.0
-- ============================================================================
...
-- 2.1 tasks 表 (任务表)
-- 
-- 说明：存储任务基本信息，通过 active_plan_id 关联当前激活的开发计划
--       一对多关系：一个任务可以有多个开发计划
-- ----------------------------------------------------------------------------
...
CREATE INDEX idx_tasks_active_plan ON tasks(active_plan_id);
...
CREATE INDEX idx_plans_reviewer ON plans(reviewer);
CREATE INDEX idx_plans_created_at ON plans(created_at);
CREATE INDEX idx_plans_reviewed_at ON plans(reviewed_at);
CREATE INDEX idx_plans_version ON plans(task_id, version);  -- 复合索引：查询任务的某个版本计划

-- 唯一约束：同一任务的同一版本只能有一个计划
CREATE UNIQUE INDEX idx_plans_task_version_unique ON plans(task_id, version);

-- ----------------------------------------------------------------------------
-- 2.3 task_plans 关联表 (任务 - 计划历史关联表)
-- 
-- 说明：记录任务与所有计划的历史关联关系，支持审计和版本追溯
--       虽然 plans.task_id 已建立外键，但此表用于记录更详细的关联元数据
-- ----------------------------------------------------------------------------
...
CREATE TABLE task_plans (
    ...
    relation_type       VARCHAR(32) NOT NULL DEFAULT 'owns',  -- 关系类型：owns/derived_from/replaced_by
    ...
    CONSTRAINT task_plans_unique_active UNIQUE (task_id, is_active) 
        EXCLUDE WHERE (is_active = true)
);

-- 索引
CREATE INDEX idx_task_plans_task_id ON task_plans(task_id);
CREATE INDEX idx_task_plans_plan_id ON task_plans(plan_id);
CREATE INDEX idx_task_plans_active ON task_plans(task_id, is_active);
CREATE INDEX idx_task_plans_relation_type ON task_plans(relation_type);

-- ----------------------------------------------------------------------------
-- 2.4 plan_steps 表 (计划步骤表 - 可选，如果 steps JSONB 不够用)
-- 
-- 说明：如果需要对步骤进行更复杂的查询和管理，可以使用此结构化表
--       否则可以直接使用 plans.steps JSONB 字段
-- ----------------------------------------------------------------------------
...
CREATE INDEX idx_plan_steps_executor ON plan_steps(executor);
...
-- ----------------------------------------------------------------------------
-- 4.1 tasks_updated_at: 自动更新 tasks 表的 updated_at 字段
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4.2 plans_updated_at: 自动更新 plans 表的 updated_at 字段 (如果需要)
-- ----------------------------------------------------------------------------

ALTER TABLE plans ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP;
...
-- ----------------------------------------------------------------------------
-- 4.3 task_plans_maintain_active: 维护 task_plans 表的激活状态
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4.4 plans_insert_task_plans: 创建计划时自动插入 task_plans 记录
-- ----------------------------------------------------------------------------
...
-- 插入示例任务 (如果需要)
-- INSERT INTO tasks (id, title, assignee, creator, priority, status)
-- VALUES ('20260416-001', '示例任务', '拉斐尔', '擎天柱', 'high', 'pending');
...
-- 设计说明:
-- 1. tasks 表存储任务基本信息，通过 active_plan_id 关联当前激活的计划
-- 2. plans 表存储开发计划，task_id 外键建立一对多关系
-- 3. task_plans 表记录完整的历史关联关系，支持审计
-- 4. 使用 JSONB 存储灵活的结构化数据 (context, steps, metadata 等)
-- 5. 视图提供便捷的查询接口，避免复杂 JOIN
-- 6. 触发器自动维护数据一致性

-- 性能优化:
-- 1. 关键查询字段建立索引 (status, assignee, deadline 等)
-- 2. 使用 GIN 索引支持 JSONB 和数组查询
-- 3. 复合索引优化常用查询组合

-- 扩展性:
-- 1. JSONB 字段支持未来新增字段无需改表结构
-- 2. plan_steps 表可选，根据实际需求使用
-- 3. 枚举类型易于扩展新状态
