"""Replace Chapter 4 SVG body projections with print-safe PNG assets.
...
EXPECTED_REVISION = 65
EXPECTED_SHA256 = "b1026dd68ce02fe65ca84dccf24fe05ede3dbf3d2a29d59281dda11dfd345bde"
ACTOR = "thesis-ch4-print-projection"

KNOWLEDGE = Path("/Users/apple/工作桌面/knowledge")
STAGE2_ROOT = KNOWLEDGE / "output/博士论文-v34第四章配图/07-stage2"
PNG_ROOT = STAGE2_ROOT / "png-projections"
REPORT_PATH = STAGE2_ROOT / "chapter4-stage2-print-projection-report.json"

FIGURES = [
    ("图4-1", "指挥意图、全局态势与任务需求的语义映射", "doc-eada07b171bb", 1),
    ("图4-2", "目标处置链、态势塑造链与共享支撑任务网络", "doc-6ed5e6d27e61", 1),
    ("图4-3", "HTN候选任务结构的生成、剪枝与全局选择", "doc-11616fa4df0c", 1),
    ("图4-4", "任务约束的继承、分层校核与不可越权关系", "doc-3608a8ce12c9", 1),
    ("图4-5", "全局态势价值约束下的MARTA兵力与资源分配结构", "doc-7f6f6f18e9a1", 2),
    ("图4-6", "分解—分配失败反馈与最小充分修正机制", "doc-bc894c778dca", 2),
    ("图4-7", "候选任务—兵力方案的人工审查、授权与交接门", "doc-b7e6d48bb6e9", 2),
]
...
def _backup(project: dict[str, Any], state: dict[str, Any]) -> Path:
    ...
    for name, value in {
        ...
        "backup-manifest.json": {
            ...
            "purpose": "chapter4-svg-to-png-print-projection",
        },
    }.items():
        path = root / name
        if not path.exists():
...
def main() -> None:
    ...
    updated = writing_collaboration_service.patch_draft(
        ...
        DOCUMENT_ID,
        {
            ...
            "client_change_id": "thesis-v34-ch4-print-png-projection-v1",
