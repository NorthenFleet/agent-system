"""Capture a read-only Chapter 5 structured-document and figure baseline."""
...
OUTPUT = Path(
    "/Users/apple/工作桌面/knowledge/output/博士论文-v34第五章配图/01-baseline/"
    "chapter5-structured-baseline.json"
)
...
def _chapter_slice(blocks: list[dict[str, Any]]) -> tuple[int, int]:
    start = next(
        ...
        and _text(block).strip().startswith("第5章")
    )
    end = next(
        index
...
def main() -> None:
    ...
    for local_index, block in enumerate(chapter):
        ...
        if block.get("type") == "image":
            images.append(row)
        if "图5-" in text or str(attrs.get("artifactLabel") or "").startswith("图5-"):
